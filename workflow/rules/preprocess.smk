...
rule convert_paired_to_interleaved:
    input:
        R1 = lambda w: data.filepath_generator(w.sample, config['raw_data']['fastq'], column="R1"),
        R2 = lambda w: data.filepath_generator(w.sample, config['raw_data']['fastq'], column="R2")
    output:
        "{dir}/{{sample}}_interleaved.fastq.gz".format(dir=config['raw_data']['fastq'])
    conda:
...
rule fastqc_pre:
    ...
    output:
        html = "{dir}/pre_trim/{{sample}}_fastqc.html".format(dir=config['fastqc']['output']),
...
use rule fastqc_pre as fastqc_post with:
    input:
        rules.fastp.output
    output:
        html = "{dir}/post_trim/{{sample}}_fastqc.html".format(dir=config['fastqc']['output']),
        zip = "{dir}/post_trim/{{sample}}_fastqc.zip".format(dir=config['fastqc']['output'])

rule error_correction:
    input:
        rules.fastp.output if config['trimming']['run'] else lambda w: data.filepath_generator(w.sample, config['raw_data']['fastq'])
    output:
        "{dir}/corrected/{{sample}}.fastq.gz".format(dir=config['raw_data']['fastq'])
    threads: 8
    resources:
...
rule deinterleave:
    ...
    output:
        R1 = "{dir}/preprocess_done/{{sample}}_R1.fastq.gz".format(dir=config['raw_data']['fastq']),
        R2 = "{dir}/preprocess_done/{{sample}}_R2.fastq.gz".format(dir=config['raw_data']['fastq'])
    params:
        min_read_length = 75
    conda:
        "envs/preprocess.yaml"
